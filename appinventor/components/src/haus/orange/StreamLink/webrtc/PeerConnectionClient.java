...
public class PeerConnectionClient {
	...
	private static final String VIDEO_FRAME_EMIT_FIELDTRIAL = PeerConnectionFactory.VIDEO_FRAME_EMIT_TRIAL + "/"
	...
	public static class PeerConnectionParameters {
		...
		public PeerConnectionParameters(boolean videoCallEnabled, boolean loopback, boolean tracing, int videoWidth,
				...
				boolean videoFlexfecEnabled, int audioStartBitrate, String audioCodec, boolean noAudioProcessing,
				boolean aecDump, boolean useOpenSLES, boolean disableBuiltInAEC, boolean disableBuiltInAGC,
				boolean disableBuiltInNS, boolean enableLevelControl, boolean disableWebRtcAGCAndHPF) {
			this(videoCallEnabled, loopback, tracing, videoWidth, videoHeight, videoFps, videoMaxBitrate, videoCodec,
					videoCodecHwAcceleration, videoFlexfecEnabled, audioStartBitrate, audioCodec, noAudioProcessing,
					aecDump, useOpenSLES, disableBuiltInAEC, disableBuiltInAGC, disableBuiltInNS, enableLevelControl,
					disableWebRtcAGCAndHPF, null);
		}
	...
	public void createPeerConnection(final VideoSink localRender, final VideoRenderer.Callbacks remoteRender,
			final VideoCapturer videoCapturer, final SignalingParameters signalingParameters) {
		createPeerConnection(localRender, Collections.singletonList(remoteRender), videoCapturer, signalingParameters);
	}

	public void createPeerConnection(final VideoSink localRender, final List<VideoRenderer.Callbacks> remoteRenders,
			final VideoCapturer videoCapturer, final SignalingParameters signalingParameters) {
	...
	private void createPeerConnectionFactoryInternal(Context context) {
		...
		if (videoCallEnabled && peerConnectionParameters.videoCodec != null) {
			...
			case VIDEO_CODEC_H264_HIGH:
				// TODO(magjed): Strip High from SDP when selecting Baseline instead of using
				// field trial.
		...
		*/
		...
		WebRtcAudioTrack.setErrorCallback(new WebRtcAudioTrackErrorCallback() {
			...
			public void onWebRtcAudioTrackStartError(AudioRecordStartErrorCode errorCode, String errorMessage) {
			...
			@Override
			public void onWebRtcAudioTrackStartError(String arg0) {
...
//		}
	...
	private void createMediaConstraintsInternal() {
		...
		if (peerConnectionParameters.noAudioProcessing) {
			...
			audioConstraints.mandatory
					.add(new MediaConstraints.KeyValuePair(AUDIO_ECHO_CANCELLATION_CONSTRAINT, "false"));
			audioConstraints.mandatory
					.add(new MediaConstraints.KeyValuePair(AUDIO_AUTO_GAIN_CONTROL_CONSTRAINT, "false"));
			audioConstraints.mandatory
					.add(new MediaConstraints.KeyValuePair(AUDIO_HIGH_PASS_FILTER_CONSTRAINT, "false"));
			audioConstraints.mandatory
					.add(new MediaConstraints.KeyValuePair(AUDIO_NOISE_SUPPRESSION_CONSTRAINT, "false"));
		}
	...
	private static String movePayloadTypesToFront(List<String> preferredPayloadTypes, String mLine) {
		// The format of the media description line should be: m=<media> <port> <proto>
		// <fmt> ...
		...
		// Reconstruct the line with |preferredPayloadTypes| moved to the beginning of
		// the payload
		// types.
	...
	private static String preferCodec(String sdpDescription, String codec, boolean isAudio) {
		...
		// A list with all the payload types with name |codec|. The payload types are
		// integers in the
