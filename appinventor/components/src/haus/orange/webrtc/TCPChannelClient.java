...
public class TCPChannelClient {
	...
	/**
	 * Callback interface for messages delivered on TCP Connection. All callbacks
	 * are invoked from the looper executor thread.
	 */
	...
	/**
	 * Initializes the TCPChannelClient. If IP is a local IP address, starts a
	 * listening server on that IP. If not, instead connects to the IP.
	 *
	...
	/**
	 * Disconnects the client if not already disconnected. This will fire the
	 * onTCPClose event.
	 */
	...
	/**
	 * Helper method for firing onTCPError events. Calls onTCPError on the executor
	 * thread.
	 */
	...
	/**
	 * Base class for server and client sockets. Contains a listening thread that
	 * will call eventListener.onTCPMessage on new messages.
	 */
	private abstract class TCPSocket extends Thread {
		...
		public void run() {
			...
			synchronized (rawSocketLock) {
				...
				try {
					out = new PrintWriter(new OutputStreamWriter(rawSocket.getOutputStream(), Charset.forName("UTF-8")),
							true);
