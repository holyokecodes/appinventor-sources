...
import haus.orange.webrtc.RecordedAudioToFileController;
...
public class PeerConnectionClient {
	...
	public static class PeerConnectionParameters {
		...
		public PeerConnectionParameters(boolean videoCallEnabled, boolean loopback, boolean tracing, int videoWidth,
				...
				boolean aecDump, boolean saveInputAudioToFile, boolean useOpenSLES, boolean disableBuiltInAEC,
				boolean disableBuiltInAGC, boolean disableBuiltInNS, boolean disableWebRtcAGCAndHPF,
	...
	public interface PeerConnectionEvents {
		...
		/**
		 * Callback fired once DTLS connection is established (PeerConnectionState is
		 * CONNECTED).
		 */
		void onConnected();

		/**
		 * Callback fired once DTLS connection is disconnected (PeerConnectionState is
		 * DISCONNECTED).
		 */
		void onDisconnected();
	 ...
	 * PeerConnectionClient takes ownership of |eglBase|.
	 */
	public PeerConnectionClient(Context appContext, EglBase eglBase, PeerConnectionParameters peerConnectionParameters,
		...
		executor.execute(() -> {
			...
			PeerConnectionFactory.initialize(PeerConnectionFactory.InitializationOptions.builder(appContext)
					.setFieldTrials(fieldTrials).setEnableInternalTracer(true).createInitializationOptions());
		});
	}
	...
	AudioDeviceModule createJavaAudioDevice() {
		...
		AudioRecordErrorCallback audioRecordErrorCallback = new AudioRecordErrorCallback() {
			...
			@Override
			public void onWebRtcAudioRecordStartError(JavaAudioDeviceModule.AudioRecordStartErrorCode errorCode,
					String errorMessage) {
		...
		AudioTrackErrorCallback audioTrackErrorCallback = new AudioTrackErrorCallback() {
			...
			@Override
			public void onWebRtcAudioTrackStartError(JavaAudioDeviceModule.AudioTrackStartErrorCode errorCode,
					String errorMessage) {
	...
	private void createMediaConstraintsInternal() {
		...
		sdpMediaConstraints.mandatory
				.add(new MediaConstraints.KeyValuePair("OfferToReceiveVideo", Boolean.toString(isVideoCallEnabled())));
	}
