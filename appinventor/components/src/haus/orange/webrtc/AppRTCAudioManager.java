...
public class AppRTCAudioManager {
	...
	public interface AudioManagerEvents {
		// Callback fired once audio device is changed or list of available audio
		// devices changed.
	...
	/**
	 * This method is called when the proximity sensor reports a state change, e.g.
	...
	private class WiredHeadsetReceiver extends BroadcastReceiver {
		...
		public void onReceive(Context context, Intent intent) {
			...
			Log.d(TAG,
					"WiredHeadsetReceiver.onReceive" + AppRTCUtils.getThreadInfo() + ": " + "a=" + intent.getAction()
							+ ", s=" + (state == STATE_UNPLUGGED ? "unplugged" : "plugged") + ", m="
	...
	public void start(AudioManagerEvents audioManagerEvents) {
		...
		// TODO(henrika): perhaps call new method called preInitAudio() here if
		// UNINITIALIZED.
		...
		// Request audio playout focus (without ducking) and install listener for
		// changes in focus.
		int result = audioManager.requestAudioFocus(audioFocusChangeListener, AudioManager.STREAM_VOICE_CALL,
	...
	/**
	 * Changes default audio device. TODO(henrika): add usage of this method in the
	 * AppRTCMobile client.
	 */
	 ...
	 * indication that audio playback is actually over the wired headset as audio
	 * routing depends on other conditions. We only use it as an early indicator
	 * (during initialization) of an attached wired headset.
	 */
	@Deprecated
	private boolean hasWiredHeadset() {
	...
	public void updateAudioDeviceState() {
		ThreadUtils.checkIsOnMainThread();
		Log.d(TAG, "--- updateAudioDeviceState: " + "wired headset=" + hasWiredHeadset + ", " + "BT state="
		...
		// Need to start Bluetooth if it is available and user either selected it
		// explicitly or
		// user did not select any output device.
		boolean needBluetoothAudioStart = bluetoothManager.getState() == AppRTCBluetoothManager.State.HEADSET_AVAILABLE
				&& (userSelectedAudioDevice == AudioDevice.NONE || userSelectedAudioDevice == AudioDevice.BLUETOOTH);
		// Need to stop Bluetooth audio if user selected different device and
		// Bluetooth SCO connection is established or in the process.
		boolean needBluetoothAudioStop = (bluetoothManager.getState() == AppRTCBluetoothManager.State.SCO_CONNECTED
				|| bluetoothManager.getState() == AppRTCBluetoothManager.State.SCO_CONNECTING)
				&& (userSelectedAudioDevice != AudioDevice.NONE && userSelectedAudioDevice != AudioDevice.BLUETOOTH);
		if (bluetoothManager.getState() == AppRTCBluetoothManager.State.HEADSET_AVAILABLE
				|| bluetoothManager.getState() == AppRTCBluetoothManager.State.SCO_CONNECTING
				|| bluetoothManager.getState() == AppRTCBluetoothManager.State.SCO_CONNECTED) {
			Log.d(TAG, "Need BT audio: start=" + needBluetoothAudioStart + ", " + "stop=" + needBluetoothAudioStop
		...
		if (bluetoothManager.getState() == AppRTCBluetoothManager.State.SCO_CONNECTED) {
			...
			// No wired headset and no Bluetooth, hence the audio-device list can contain
			// speaker
			// phone (on a tablet), or speaker phone and earpiece (on mobile phone).
