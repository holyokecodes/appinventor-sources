...
public class DirectRTCClient implements AppRTCClient, TCPChannelClient.TCPChannelEvents {
	...
	// All alterations of the room state should be done from inside the looper
	// thread.
	...
	/**
	 * Connects to the room, roomId in connectionsParameters is required. roomId
	 * must be a valid IP address matching IP_PATTERN.
	 */
	@Override
	...
	public void onTCPConnected(boolean isServer) {
		if (isServer) {
			...
			SignalingParameters parameters = new SignalingParameters(
					...
					new ArrayList<>(), isServer, // Server side acts as the initiator on direct connections.
					null, // clientId
					null, // wssUrl
					null, // wwsPostUrl
					null, // offerSdp
	...
	public void onTCPMessage(String msg) {
		try {
			...
			if (type.equals("candidate")) {
				...
				SignalingParameters parameters = new SignalingParameters(
						// Ice servers are not needed for direct connections.
						new ArrayList<>(), false, // This code will only be run on the client side. So, we are not the
													// initiator.
						null, // clientId
						null, // wssUrl
						null, // wssPostUrl
